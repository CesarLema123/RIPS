...
run ${NLOOP}
print "$i"
#print "$(abs(v_volumeSTD-v_volumeSTD1)>(v_volumeErr*v_volumeSTD1)) || $(abs(v_pressSTD - v_pressSTD1)) > $(v_pressErr*v_pressSTD1) || $(abs(v_energySTD - v_energySTD1)) > $(v_energyErr*v_energySTD1) ||  $(abs(v_varAveTemp - v_TEMPERATURE)) > $(v_tempErr*v_TEMPERATURE)"
if "$(abs(v_volumeSTD-v_volumeSTD1)>(v_volumeErr*v_volumeSTD1)) || $(abs(v_pressSTD - v_pressSTD1)) > $(v_pressErr*v_pressSTD1) || $(abs(v_energySTD - v_energySTD1)) > $(v_energyErr*v_energySTD1) ||  $(abs(v_varAveTemp - v_TEMPERATURE)) > $(v_tempErr*v_TEMPERATURE)" then "variable i delete" "variable i loop ${loopExit}" "jump convergence.mod set_vars" 
