...
def getFinalStats(fileName):
	...
	This should be used when you have used the initConv.mod before the data collection run. 
	This is not done automatically and must be changed in an in.*** lammps file 
    """
    df = readLog(fileName)
...
def computeStats(data):
	"""
	This function breaks up the data into bins and computes the average of the whole set, the std of the whole set, and the std of the averages
	over the bins.
	"""
	N = len(data)
	nBins = 20
...
def getThermoStats(fileName):
    """
    This Function reads log file and outputs the average, standard deviation and standard deviaton of the mean for each thermodynamic variable
	It runs like computeStats but over a full log file.
	It assumes that you have used initConv.mod before the data collection run.
    """
    df = readLog(fileName)
...
def getThermoStats1(fileName):
	...
	This function is like getThermoStats but it does not assume that you have used the initConv.mod before the data collection run.
	However, it computes the statistics of every file and so some of the rows are likely to be meaningless.
    """
    df = readLog(fileName)
