...
FILE = "data.Cu" + str(num)
HEADER = "Copper Diffusion Simulation"

STYLE = "atomic" 

INITTEMP = 1000 # kelvin (set via random inital velocities
KB = 8.617333262e-5
...
NUM_ATOMS = NUMX*NUMY*NUMZ + (NUMX-1)*(NUMY-1)*(NUMZ) + (NUMX-1)*(NUMY)*(NUMZ-1) + (NUMX)*(NUMY-1)*(NUMZ-1)
