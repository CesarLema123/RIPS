...
def writeDataFile(wFile = "data.CuNi",style = "atomic", latticeType = "fcc", initTemp = 300,nAtoms = [6,6,6],latticeParams = [3.6,3.6,3.6],concFunc = lambda a: 1 if a < 0.3 else 2,header = "Lammps Data File for CuNi",nBonds = 0,nAngles = 0,nDihedrals = 0,nImpropers = 0,nAtomTypes = 1,nBondTypes = 0,nAngleTypes = 0,nDihedralTypes = 0,nImproperTypes = 0):
...
def writeInNPT(runTime = 1000,timeStep = 0.0001,temp = 300,press = 0,readFile = "in.Template", writeFile = "in.CuNi"):
...
def writeInNVT(runTime = 1000,timeStep = 0.0001,temp = 300,length = 21.0,readFile = "in.Template", writeFile = "in.CuNi",dataFile = "data.CuNi"):
    reader = open(READFILE)
    writer = open(WRITEFILE,mode="w")
    simSteps = str(int(runTime/timeStep))
    writer.write("# --------------- Define Variables --------------\n")
