...
class DataFrameAnalyzer:
    ...
    def concInterfaceEng(self):
        TE1 = self.df['TEng1'].values.astype(float)
        TE2 = self.df['TEng2'].values.astype(float)
        TEM = self.df['TEngMix'].values.astype(float)
        midIndex = (len(TE1) - 1)//2
        numTerms = len(TE1[midIndex:])
        avgTE1 = np.sum(TE1[midIndex:])/numTerms
        print('The average energy for c1 is: ', avgTE1)
        avgTE2 = np.sum(TE2[midIndex:])/numTerms
        print('The average energy for c2 is: ', avgTE2)
        avgTEM = np.sum(TEM[midIndex:])/numTerms
        print('The average energy for cmixed is: ', avgTEM)
